...
# Test email access
mcporter call ms365.list_messages top=5
...
Direct mcporter usage (mcporter converts underscores to hyphens automatically):
```bash
# List emails
mcporter call ms365.list_messages top=10

# List calendar events
mcporter call ms365.list_events top=5

# Note: Use underscores in mcporter calls (list_messages),
# but hyphens in direct MCP tool names (list-mail-messages)
```
...
The `scripts/` directory contains utilities for managing the MS365 MCP server:

- **`auth-device.sh`** - Interactive device code authentication for MS 365
- **`check-auth.sh`** - Check authentication status and test connection
- **`list-tools.sh`** - List all available MS 365 tools from the MCP server
- **`start-server.sh`** - Start MS 365 MCP server in HTTP mode (supports org-mode, read-only, TOON format)
...
# Start HTTP server on port 3365
...
**Note**: When calling via mcporter, replace hyphens with underscores in tool names (e.g., `list-mail-messages` becomes `list_messages`).
...
| `reply-to-mail-message` | Reply to an email |
| `forward-mail-message` | Forward an email |
...
| `delete-file-or-folder` | Delete file/folder |
